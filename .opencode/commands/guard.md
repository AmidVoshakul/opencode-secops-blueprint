...
2. **Context Weaponization:** Read `.opencode/instructions/security-threat-intel.md`. Use the real CVEs and attacks listed there (like ClawJacked, Symlink escapes, Microsoft EchoLeak) as blueprints to look for matching flaws in our code.
...
1. Scan all input-handling modules, configuration parsers, and string manipulation utilities across the crates.
...
3. Verify that all system execution wrappers use pre-compiled arrays (`Command::new`) and completely ban raw shell dynamic string interpretation (Anti-MCP05).

## Phase 2: Sandbox, Permissions & State Audit (ASI03)
Specifically target your analysis at `nest-sandbox`, `nest-permissions`, and `nest-audit`:
1. Check if the Linux namespaces, `pivot_root`, and `seccomp-bpf` syscall hooks are fully safe from symlink race-condition escapes (targeting CVE-2026-39861).
...
1. Audit `nest-messaging` and inter-agent transport contracts. Look for race conditions that could lead to unauthorized state modification or session smuggling (A2A protocol flaws).
2. Trace token storage and validation routes in memory to block cross-agent credential leaks.
...
3. Output the "SecOps Adversarial Vulnerability Report" structured into:
   ...
   - **Immediate Security Hardening Patch (Ready-to-apply Rust code fixes)**
