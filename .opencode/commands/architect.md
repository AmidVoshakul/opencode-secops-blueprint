...
2. Every file created must contain clean boilerplate code with proper imports, type annotations, and basic error handling stubs.
...
4. **MANDATORY PRIMARY AGENT HANDOVER (CONTEXT BRIDGE):** At the absolute end of your response, you MUST generate a separate, isolated block enclosed in `---CONTEXT-BRIDGE---` markers. Write a dense, structured technical summary of the exact directory paths created, names of core boilerplate files generated, and public trait/interface identifiers introduced. This ensures the primary agent instantly captures the structural additions and can immediately help write implementations or test scenarios.
