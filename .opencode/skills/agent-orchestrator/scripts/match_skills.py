...
def sanitize_string(s: str) -> str:
    """Sanitize queries to mitigate prompt boundary breakouts and injection scripts."""
    if not s:
        return ""
    bad_chars = [";", "&&", "|", "`", "$", "(", ")", '"']
...
def ensure_registry():
    """Run scan securely if registry doesn't exist."""
...
def query_to_capabilities(query: str) -> set[str]:
    """Map a query to unique capability categories using boundary deduction."""
    q_lower = query.lower()
...
def score_skill(skill: dict, query: str, project_skills: set = None) -> dict:
    """Score a skill's relevance to an isolated sanitized query."""
    q_lower = query.lower()
    score = 0
    reasons = []
...
def match(query: str, project: str = None, top_n: int = 5, threshold: int = 5) -> list[dict]:
    """Match a query against all registered clean skills from history."""
...
def main():
    ...
    query_clean = sanitize_string(raw_query)
    ...
    results = match(query_clean, project=project)
