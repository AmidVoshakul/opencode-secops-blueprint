...
```bash
uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/match_skills.py "<solicitacao do usuario>"
```
...
```bash
uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/orchestrate.py --skills skill1,skill2 --query "<solicitacao>"
```
...
```bash
uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/scan_registry.py && uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/match_skills.py "<solicitacao>"
```
...
O registry vive em:
```
.opencode/skills/agent-orchestrator/data/registry.json
```

## Locais De Busca

O scanner procura SKILL.md em:
1. `.opencode/skills/*/` (skills registradas no OpenCode)
...
uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/scan_registry.py --force
```
...
```bash
uv run --with "pyyaml>=6.0" python .opencode/skills/agent-orchestrator/scripts/match_skills.py --project meu-projeto "query aqui"
```
...
```
.opencode/skills/agent-orchestrator/data/projects.json
```
...
4. Copiar o SKILL.md para `.opencode/skills/<nome>/SKILL.md`
